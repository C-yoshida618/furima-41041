  ...
  context '新規登録がうまくかないとき' do
    ...
    it 'パスワードが空欄だと保存できない' do
      ...
      expect(@user.errors.full_messages).to include("Password can't be blank", "Password confirmation doesn't match Password",
                                                    "Password can't be blank", 'Password is invalid')
    end
    ...
    it '性(全角)が空欄だと保存できない' do
      @user.last_name = ''
      @user.valid?
      expect(@user.errors.full_messages).to include("Last name can't be blank")
    end

    it '名(全角)が空欄だと保存できない' do
      @user.first_name = ''
      @user.valid?
      expect(@user.errors.full_messages).to include("First name can't be blank")
    end

    it '性(カナ)が空欄だと保存できない' do
      @user.last_name_kana = ''
      @user.valid?
      expect(@user.errors.full_messages).to include("Last name kana can't be blank")
    end

    it '名(カナ)が空欄だと保存できない' do
    ...
    it '全角文字を含むパスワードでは登録できない' do
      @user.password = 'describe'
      @user.password_confirmation = 'describe'
      @user.valid?
      expect(@user.errors.full_messages).to include('Password is invalid')
    end
